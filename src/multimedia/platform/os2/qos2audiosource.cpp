...
QOS2AudioSource::QOS2AudioSource(const QByteArray &device)
{
    bytesAvailable = 0;
//    handle = 0;
...
QOS2AudioSource::~QOS2AudioSource()
{
    close();
...
void QOS2AudioSource::setVolume(qreal vol)
{
    m_volume = vol;
}

qreal QOS2AudioSource::volume() const
{
    return m_volume;
}

QAudio::Error QOS2AudioSource::error() const
{
    return errorState;
}

QAudio::State QOS2AudioSource::state() const
{
    return deviceState;
}

void QOS2AudioSource::setFormat(const QAudioFormat& fmt)
{
    if (deviceState == QAudio::StoppedState)
        settings = fmt;
}

QAudioFormat QOS2AudioSource::format() const
{
    return settings;
}

int QOS2AudioSource::xrun_recovery(int err)
{
    int  count = 0;
    bool reset = false;
#if 0
...
void QOS2AudioSource::start(QIODevice* device)
{
    if(deviceState != QAudio::StoppedState)
        close();
...
QIODevice* QOS2AudioSource::start()
{
    if(deviceState != QAudio::StoppedState)
        close();
    ...
    pullMode = false;
    audioSource = new OS2InputPrivate(this);
...
int QOS2AudioSource::checkBytesReady()
{
#if 0
    if(resuming)
...
int QOS2AudioSource::bytesReady() const
{
    return qMax(bytesAvailable, 0);
}

qint64 QOS2AudioSource::read(char* data, qint64 len)
{
#if 0
...
void QOS2AudioSource::setBufferSize(int value)
{
    buffer_size = value;
}

int QOS2AudioSource::bufferSize() const
{
    return buffer_size;
}

qint64 QOS2AudioSource::processedUSecs() const
{
...
bool QOS2AudioSource::deviceReady()
{
    if(pullMode) {
        ...
        OS2InputPrivate* a = qobject_cast<OS2InputPrivate*>(audioSource);
        a->trigger();
    }
    bytesAvailable = checkBytesReady();
...
OS2InputPrivate::OS2InputPrivate(QOS2AudioSource* audio)
{
    audioDevice = qobject_cast<QOS2AudioSource*>(audio);
}

OS2InputPrivate::~OS2InputPrivate()
{
}

qint64 OS2InputPrivate::readData( char* data, qint64 len)
{
    return audioDevice->read(data,len);
}

qint64 OS2InputPrivate::writeData(const char* data, qint64 len)
{
    Q_UNUSED(data);
    Q_UNUSED(len);
    return 0;
...
#include "moc_qos2audiosource_p.cpp"
