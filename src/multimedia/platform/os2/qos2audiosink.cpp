...
#include "qos2audiosink_p.h"
#include "qos2audiodevice_p.h"
#include <QLoggingCategory>

QT_BEGIN_NAMESPACE

Q_LOGGING_CATEGORY(lcOS2Output, "qt.multimedia.os2.output")
//#define DEBUG_AUDIO 1

QOS2AudioSink::QOS2AudioSink(const QByteArray &device)
{
    bytesAvailable = 0;
//    handle = 0;
...
void QOS2AudioSink::setVolume(qreal vol)
{
    m_volume = vol;
}

qreal QOS2AudioSink::volume() const
{
    return m_volume;
}

QAudio::Error QOS2AudioSink::error() const
{
    return errorState;
}

QAudio::State QOS2AudioSink::state() const
{
    return deviceState;
}

int QOS2AudioSink::xrun_recovery(int err)
{
    int  count = 0;
    bool reset = false;
#if 0
...
void QOS2AudioSink::start(QIODevice* device)
{
...
QIODevice* QOS2AudioSink::start()
{
    ...
    audioSource = new OS2OutputPrivate(this);
...
bool QOS2AudioSink::open()
{
    ...
    if ( !fatal ) {
        ...
        if ( err < 0 ) {
            ...
            qCDebug(lcOS2Output) << "buffer time: [" << minBufferTime << "-" << maxBufferTime << "] =" << buffer_time;
            qCDebug(lcOS2Output) << "period time: [" << minPeriodTime << "-" << maxPeriodTime << "] =" << period_time;
            qCDebug(lcOS2Output) << "chunks =" << chunks;
        }
    }
    if ( !fatal ) {
...
qsizetype QOS2AudioSink::bytesFree() const
{
#if 0
    if(resuming)
        return period_size;
...
qint64 QOS2AudioSink::write( const char *data, qint64 len )
{
#if 0
...
void QOS2AudioSink::setBufferSize(qsizetype value)
{
    if(deviceState == QAudio::StoppedState)
        buffer_size = value;
}

qsizetype QOS2AudioSink::bufferSize() const
{
    return buffer_size;
}

qint64 QOS2AudioSink::processedUSecs() const
{
    return qint64(1000000) * totalTimeValue / settings.sampleRate();
}

void QOS2AudioSink::resume()
{
#if 0
...
void QOS2AudioSink::setFormat(const QAudioFormat& fmt)
{
    if (deviceState == QAudio::StoppedState)
        settings = fmt;
}

QAudioFormat QOS2AudioSink::format() const
{
    return settings;
}

void QOS2AudioSink::suspend()
{
...
bool QOS2AudioSink::deviceReady()
{
#if 0
    if(pullMode) {
        int l = 0;
...
OS2OutputPrivate::OS2OutputPrivate(QOS2AudioSink* audio)
{
    audioDevice = qobject_cast<QOS2AudioSink*>(audio);
}

OS2OutputPrivate::~OS2OutputPrivate() {}

qint64 OS2OutputPrivate::readData( char* data, qint64 len)
{
    Q_UNUSED(data);
    Q_UNUSED(len);
...
qint64 OS2OutputPrivate::writeData(const char* data, qint64 len)
{
    int retry = 0;
    qint64 written = 0;
...
#include "moc_qos2audiosink_p.cpp"
